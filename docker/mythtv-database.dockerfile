...
RUN apt-get update \
    ...
    && echo "deb http://ppa.launchpad.net/mythbuntu/${PPA_NAME}/ubuntu focal main" >> /etc/apt/sources.list \
    && echo "deb-src http://ppa.launchpad.net/mythbuntu/${PPA_NAME}/ubuntu focal main" >> /etc/apt/sources.list \
    && apt-get update \
